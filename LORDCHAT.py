...
# Function to move the head in all directions
def test_head_movement():
    print("Testing head movement...")
    positions = [3000, 6000, 9000]  # Min, Center, Max positions

    for pos in positions:
        servo.setTarget(HEAD_TILT, pos)  # Tilt Up, Center, Down
        time.sleep(1)

    for pos in positions:
        servo.setTarget(HEAD_PAN, pos)  # Pan Left, Center, Right
        time.sleep(1)
...
def turn_90_degrees():
    print("Turning robot 90 degrees...")
    servo.setTarget(MOTOR_LEFT, 7000)   # Forward left motor
    servo.setTarget(MOTOR_RIGHT, 5000)  # Reverse right motor
    time.sleep(1.5)  # Adjust time for proper turning
    servo.setTarget(MOTOR_LEFT, 6000)   # Stop motors
    servo.setTarget(MOTOR_RIGHT, 6000)
    print("Turn complete.")

# Function to detect a colored object and move forward
