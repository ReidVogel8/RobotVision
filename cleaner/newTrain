...
print("Draw a box around the object, press 's' to save one view, 'n' to finish this object.")
...
try:
    while object_count < MAX_OBJECTS:
        ...
        cv2.putText(display, f"Object {object_count + 1}/{MAX_OBJECTS}: 's' = save view, 'n' = next object",
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

        cv2.imshow('Training Mode', display)
        key = cv2.waitKey(1) & 0xFF

        # Initialize if first view of this object
        if object_count == len(trained_objects):
            name = input(f"Enter name for object {object_count + 1}: ")
            obj_id = object_count + 1
        ...
        if key == ord('s') and current_roi:
            ...
            # Save keypoints and descriptors for this view
            trained_objects[object_count]['keypoints'].extend([
                (kp.pt, kp.size, kp.angle, kp.response, kp.octave, kp.class_id)
                for kp in keypoints
            ])
            trained_objects[object_count]['descriptors'].append(descriptors)

            print(f"View saved for object '{trained_objects[object_count]['name']}'")

        elif key == ord('n'):
            if not trained_objects[object_count]['descriptors']:
                print("No views saved. Save at least one view before moving to next object.")
                continue
            print(f"Finished training object '{trained_objects[object_count]['name']}'")
            object_count += 1
            current_roi = None
...
if len(trained_objects) == MAX_OBJECTS:
    # Combine all descriptor arrays for each object
    for obj in trained_objects:
        obj['descriptors'] = np.vstack(obj['descriptors'])
